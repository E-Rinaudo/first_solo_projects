...
>>>>>>> 317436f (Make the repository.)
    ...
    def _make_fleet(self):
        ...
        if (
            (random() < self.settings.orca_frequency) 
            and (len(self.orcas) < self.settings.max_orcas)
        ):
            orca = Orca(self)
            self.orcas.add(orca)
    ...
    def _check_orcas_bottom(self):
        ...
        for orca in self.orcas.sprites():
            ...
            if orca.rect.bottom >= (self.settings.screen_height 
                        - self.scoreboard_button.height):
    ...
    def _update_orca_bullets(self):
        ...
        for orca_bullet in self.orca_bullets.copy():
            ...
            if orca_bullet.rect.top >= (self.settings.screen_height 
                        - self.scoreboard_button.height):
